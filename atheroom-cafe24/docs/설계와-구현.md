...
프런트엔드는 TypeScript 검사와 Next.js production build를 실행한다. 이 환경에서 실제 브라우저 조작·시각 비교 테스트는 실행하지 않았다. 테스트용 시뮬레이터 성공은 실제 Cafe24 응답 형식과 앱 권한 검증을 대신하지 않는다. 자세한 실행 기록과 남은 실계정 검증은 `검증결과.md`에 기록한다.
